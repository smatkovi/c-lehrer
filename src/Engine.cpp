...
QString configDir()
{
    // Named after the running application, not hard-coded: the same binary
    // serves several courses, and two courses sharing one progress file
    // would show each other's level and mark each other's lessons done.
    QString name = QCoreApplication::applicationName();
    if (name.isEmpty())
        name = QLatin1String("c-lehrer");
    return QDir::homePath() + QLatin1String("/.config/") + name;
}
